...
The Hub provides documentation, a machine-readable [tool registry](TOOL_REGISTRY.json), and a local validator. The remaining twelve entries are `Planned` in the Hub and await independent delivery evidence. The original ten-tool order is preserved; CFML Check, Result Store, and Runtime Trace were appended on 2026-09-07 with [detailed draft specifications](docs/TOOL_EXPANSION.md). These specifications are not implementations. Change Impact's observed scaffold does not count as another completed tool.
