...
The Hub provides documentation, a machine-readable [tool registry](TOOL_REGISTRY.json), and a local validator. The remaining thirteen entries are not completed in the Hub. The original ten-tool order is preserved; CFML Check, Result Store, and Runtime Trace were appended on 2026-09-07 with [detailed specifications](docs/TOOL_EXPANSION.md). Symbol Search was appended on 2026-09-08 after an implementation admission review and remains `Experimental`; its package `0.1.0` is not published. CFML Check now has a confirmed public repository and a local feasibility implementation, but its Hub lifecycle remains `Planned` until maintainer review and admission evidence are confirmed. Change Impact's observed scaffold does not count as another completed tool.
