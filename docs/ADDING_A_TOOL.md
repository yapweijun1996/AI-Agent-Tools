...
4. **Align scope and priority.** Update [Roadmap](../ROADMAP.md) if adding or changing priority. The original ten-tool order is preserved; three owner-requested additions are documented in the [expansion review](TOOL_EXPANSION.md). Further additions or reorderings require an explicit reviewed rationale. Do not turn priority into a tool dependency.
...
Synchronize changed tool status and design to the existing Company KB using
[KB synchronization](KB_SYNC.md). Update an existing canonical status record in
place, keep detailed design separately, and read back the affected records before
claiming synchronization complete. KB assertions do not replace a release audit.
